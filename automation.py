...
def setup_logging(log_file: Optional[str] = None):
    ...
    if log_file is None:
        ...
        if platform.system() == 'Linux':
            ...
            log_dir = os.path.expanduser('~/Library/Logs/VisualWarnings')
        ...
        if not os.path.exists(log_dir):
            try:
                ...
                # Fallback to local directory if can't create log dir
                log_dir = '.'

        log_file = os.path.join(
            log_dir, f"visual_warnings_automation_{datetime.now().strftime('%Y%m%d')}.log")
...
"""
        ...
        with open(image_path, 'rb') as img_file:
            ...
            # Send the request
            logger.info(
                f"Sending {event_type} image and text to webhook: {os.path.basename(image_path)}")
            ...
            if 'polygon' in alert and alert['polygon']:
                ...
                try:
                    ...
                    if image_path:
                        ...
                        logger.info(
                            f"Generated map for {event_type}: {image_path}")

                        # Send to webhook if URL provided
                        if webhook_url:
                            send_image_webhook(
                                image_path, webhook_url, alert, logger)
                    else:
                        logger.warning(
                            f"Failed to generate map for {alert_id}")
                except Exception as e:
...
def run_scheduled_job(custom_feed_url: str, output_dir: str, webhook_url: Optional[str], logger: logging.Logger):
    ...
    try:
        image_paths = check_and_generate_warnings(
            custom_feed_url, output_dir, webhook_url, logger)
        logger.info(
            f"Scheduled job completed. Generated {len(image_paths)} images.")
    except Exception as e:
...
def cleanup_old_files(output_dir: str, max_age_hours: int, logger: logging.Logger):
    ...
    try:
        ...
        if count > 0:
            logger.info(
                f"Cleaned up {count} files older than {max_age_hours} hours")
    except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Automate NWS warning image generation')
    ...
    parser.add_argument('--interval', type=int, default=1,
                        help='Check interval in minutes (default: 1 minute)')
    parser.add_argument('--run-once', action='store_true',
                        help='Run once and exit')
    parser.add_argument('--webhook', type=str,
                        help='Webhook URL to send images to')
    parser.add_argument('--max-age', type=int, default=48,
                        help='Maximum age of files in hours before cleanup (default: 48 hours)')
    ...
    if args.run_once:
        ...
        check_and_generate_warnings(
            args.feed_url, args.output, args.webhook, logger)
            ...
            while True:
                try:
                    ...
                    if consecutive_errors >= max_consecutive_errors:
                        logger.critical(
                            f"Too many consecutive errors ({consecutive_errors}). Restarting scheduler...")
